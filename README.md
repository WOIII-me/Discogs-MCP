...
- **Evidence dossiers**: `find_best_pressing` / `compare_pressings` return a full dossier per pressing, not just a number — the concrete signals found, mastering credits, matrix/runout, a one-line `whyItScores`, an `evidenceCoverage` (0–1) showing how well-supported the score is, and a provisional `verdict`. When the optional Jev integration is enabled on a deployment, dossiers also carry `catalogClaims`: a semantic reading of the release notes (analog/digital source, DMM, pressing plant, QC complaints, promo/test pressing, first-pressing claims) as stated / denied / contradictory with the cited sentence. Claims are annotations only and never change scores; a deployment can restrict them to a beta allowlist of Discogs accounts (`JEV_BETA_USERS`). Verdicts are provisional: read them alongside coverage, and treat the scoring as reputation/community-data-based, not measured sound (the response's `dataCaveats` spell this out).
