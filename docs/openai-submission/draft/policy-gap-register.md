...
| PG-08 | The production MCP origin and OAuth resource identifier are not final. | Blocking; tool scan, OAuth review, and domain verification require stable production identifiers. | Owner-approved HTTPS origin, deployed OAuth metadata, and domain-verification evidence. | Project owner + engineering | Decision required |
