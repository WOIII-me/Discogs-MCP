...
| PG-02 | `https://woiii.me/privacy` returns HTTP 200, but the public preflight detects missing Cloudflare and username/rating disclosures plus conflicting absolute no-collection/no-sharing claims. | High; the policy may not accurately disclose data categories, purposes, recipients, retention, and user controls. | Owner-approved policy reviewed against the final data-flow inventory and a strict `npm run check:surfaces` pass. | Project owner | Revision required |
| PG-03 | `https://woiii.me/terms` does not provide usable terms content or return HTTP 200. | Blocking; the listing currently has no public Terms of Service URL. | Owner-approved HTTPS terms page matching the submitted product/publisher and a strict `npm run check:surfaces` pass. | Project owner | Missing |
| PG-04 | `https://woiii.me/support` does not provide usable support content, and repository artifacts use both `support@woiii.me` and `help@woiii.me`. | Blocking; the canonical support route/contact is unresolved. | Owner-selected support contact, tested delivery, public support page with secret-handling guidance, consistent listing/manifest/site values, and a strict `npm run check:surfaces` pass. | Project owner | Decision required |
